...
| Shift columns instead of adding | Every browser binds at construction. On, Columns view keeps three slots on screen, divides the viewport between them, shows the focused folder in the trailing slot, and opens a location with its parent already beside it. Off restores free-growing columns at their manually resized widths. Viewports too narrow to divide fall back to free-growing columns regardless of the saved value. |
